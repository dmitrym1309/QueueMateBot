...
2. **Создайте файл `.env`** в корневой директории проекта. Вы можете скопировать `.env.example` в `.env` и затем указать ваш `BOT_TOKEN`:
   ```
   BOT_TOKEN=ваш_уникальный_токен_бота
   ```
   Это **обязательный шаг** для аутентификации бота. Более подробную информацию о настройке `.env` и получении токена можно найти в файле `docs/installation.md`.

3. Запустите бота с помощью Docker Compose:
   ```bash
   docker-compose up -d
   ```
   Эта команда также настроит Docker-том для сохранения данных бота (базы данных SQLite и лог-файлов), обеспечивая их сохранность между перезапусками контейнера.
